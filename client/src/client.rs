...
fn opt_into_json<T: Sized + Send>(opt: Option<T>) -> Result<serde_json::Value>
where
    T: serde::ser::Serialize,
{
    match opt {
...
async fn opt_result<T: for<'a> serde::de::Deserialize<'a>>(
...
pub trait RpcApi: Sized + Sync + Send {
    ...
    async fn get_block_info_with_txs(
        &self,
        hash: &bitcoin::BlockHash,
    ) -> Result<json::GetBlockWithTxsResult> {
    ...
    async fn get_tx_out(
        ...
        opt_result(self.call("gettxout", handle_defaults(&mut args, &[null()])).await?).await
    }
    ...
    pub fn new(url: &str, auth: Auth) -> Result<Self> {
        ...
        match (user, pass) {
            (Some(user), pass) => {
                ...
                url.set_password(pass.as_deref()).unwrap();
            }
            _ => {}
        }
...
impl RpcApi for Client {
    ...
    async fn call<T: for<'a> serde::de::Deserialize<'a>>(
        ...
        let req_body = Client::build_request_body(cmd, &raw_args);
